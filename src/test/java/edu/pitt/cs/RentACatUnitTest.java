...
public class RentACatUnitTest {
	...
	public void setUp() throws Exception {
		...
		// TODO: Fill in
		r = RentACat.createInstance(InstanceType.IMPL); // Use real implementation for unit testing
	...
	public void testGetCatNullNumCats0() throws NoSuchMethodException, SecurityException, IllegalAccessException, InvocationTargetException {
		...
		assertNull("getCat should return null when no cats are added", result);
	...
	public void testGetCatNumCats3() throws NoSuchMethodException, SecurityException, IllegalAccessException, InvocationTargetException {
		...
		String test = c2.toString();

		Method getCatMethod = r.getClass().getDeclaredMethod("getCat", int.class);
    	getCatMethod.setAccessible(true);
		Cat result = (Cat) getCatMethod.invoke(r, 2);
		assertNotNull("getCat should not return null when the cat exists", result);
    	assertEquals("Retrieved cat should have ID 2", 2, result.getId());
	...
	public void testListCatsNumCats0() {
		// TODO: Fill in
		assertEquals("listCats should return an empty string when there is no cat in the list", "", r.listCats());
	}
	...
	public void testListCatsNumCats3() {
		...
		assertEquals("listCats should correctly list all cats", expectedOutput, r.listCats());
	...
	public void testRenameFailureNumCats0() {
		...
		assertFalse("renameCat should return false when there is no cat", result);
    	assertEquals("System output should indicate invalid cat ID", "Invalid cat ID." + newline, out.toString());
	...
	public void testRenameNumCat3() {
		...
		r.addCat(c3);
		boolean result = r.renameCat(2, "Garfield");
		assertTrue("renameCat should return true when the cat exists and can be renamed", result);
    	Mockito.verify(c2).renameCat("Garfield"); // Verify that renameCat was indeed called on the mock object c2
	...
	public void testRentCatNumCats3() {
		...
		assertTrue("rentCat should return true when the cat is available for rent", result);
		assertEquals("Expected system output when cat is rented", "Old Deuteronomy has been rented." + newline, out.toString());
		Mockito.verify(c2).rentCat(); // Verify that rentCat was called on the mock object c2
	...
	public void testRentCatFailureNumCats3() {
		...
		boolean result = r.rentCat(2);
		assertFalse("rentCat should return false when the cat is already rented", result);
    	assertEquals("Expected system output when cat is already rented", "Sorry, Old Deuteronomy is not here!" + newline, out.toString());
	...
	public void testReturnCatNumCats3() {
		...
		assertTrue("returnCat should return true when the cat is successfully returned", result);
		assertEquals("Expected system output when cat is returned", "Welcome back, Old Deuteronomy!" + newline, out.toString());
		Mockito.verify(c2).returnCat(); // Verify that returnCat was called on the mock object c2
	...
	public void testReturnFailureCatNumCats3() {
		...
		assertFalse("returnCat should return false when the cat was not rented", result);
		assertEquals("Expected system output when cat is not rented", "Old Deuteronomy is already here!" + newline, out.toString());
		// No need to verify returnCat on c2 (It has not been called)
	}
